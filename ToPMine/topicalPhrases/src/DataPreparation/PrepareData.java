...
public class PrepareData{
	...
	//first pass is to find rare words
		...
		InstanceList instances =  createInstance(true, rareWordFile,stopwordFile, lang);
    ...
    //second pass is to do the real preprocessing
		...
		instances =  createInstance(false, rareWordFile,stopwordFile, lang);
	...
	**/
	...
	public static InstanceList createInstance(boolean isFirstPass, String rareWordFile,String stopwordFile, String lang){
		 // Begin by importing documents from text to feature sequences
        ArrayList<Pipe> pipeList = new ArrayList<Pipe>();
		...
		if(lang.equals("en")){
			...
			if(isFirstPass){
				pipeList.add( new SpanishStemming(null, counter)); // at first pass, i only need to find rare words, so no need to record the unstemmed version
			}else{
				pipeList.add( new SpanishStemming(voc)); // do stemming and at the same time find the unstemmed version
				pipeList.add( new TokenSequenceRemoveStopwords(new File(rareWordFile), "UTF-8", false, false, false) );
			}
		}
...
class SpanishStemming extends Pipe implements Serializable {
	HashMap<String, HashMap<String,Integer>> voc;
	HashMap<String,Integer> counter ;

	public SpanishStemming(HashMap<String, HashMap<String,Integer>> voc, HashMap<String,Integer> counter) {
		this.voc = voc;
		this.counter = counter;
	}
	
	public SpanishStemming(HashMap<String, HashMap<String,Integer>> voc) {
		this.voc = voc;
		this.counter = null;
	}/**/
